...
const CollabEntry = () => {
  ...
  useEffect(() => { sessionStorage.removeItem('collab_locked_room') }, [])

  const handleGo = () => {
    if (!trimmed) return
    navigate(`/collab/${encodeURIComponent(trimmed)}`)
  }
        ...
        <div style={{ position:'relative', borderRadius:24, padding:2 }}>
          ...
          <div style={{
            ...
            <div style={{ marginTop:16, fontSize:22, fontWeight:900 }}>
              <span className="g-text">Cloud Save + Playback</span>
            </div>
            <div style={{ marginTop:6, fontSize:13, color:'#6b7280', lineHeight:1.55 }}>
              Saves snapshots to Supabase with{' '}
              <b style={{ color:'#7c3aed' }}>auto-checkpoints</b> every 15 seconds.
              Replay them anytime in Playback mode.
            </div>
